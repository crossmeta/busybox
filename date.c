...
#ifdef _WIN32
{
	char *ptr = strdup(date_fmt);	/* since it is in .text rdonly */
	date_fmt = ptr;
	for (date_fmt = ptr; *ptr; ptr++)
		if ((*ptr == '%') && (*(ptr+1) == 'e')) {
			*(ptr+1) = 'd';
			break;
		}
...
#ifdef _WIN32
{
	char *ptr;
	char *np;
	char *t_zone = xmalloc(101);

	/*
	 * We are trying to abbreviate the timezone information 
	 * Pacific Standard Time to PST
	 * But Windows10 msvcrt seem to handle %z fine. The check of length > 3
	 * may not be adequate.
	 */
	strftime(t_zone, 200, "%Z", &tm_time);
	if (strlen(t_zone) > 3 && (ptr = strstr(t_buff, t_zone))) {
		int i = 0;
		np = ++ptr;
		while (*ptr && i++ < 2) {
			while (!isspace(*ptr))
		...
		/* Skip over to last space and copy the year */
		while (!isspace(*ptr))
			ptr++;
