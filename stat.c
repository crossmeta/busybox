...
#define	IF_SELINUX(x)
#define	IF_FEATURE_STAT_FORMAT(x)	x
#define	IF_FEATURE_STAT_FILESYSTEM(x)	x

#define	bb_mode_string(m)	mode_string(m)
static __inline
...
{
	char *buf = xreadlink(path);
	if (!buf) {
	/* EINVAL => "file: Invalid argument" => puzzled user */
		error_msg("%s: cannot read link (not a symlink?)", path);
	}
	return buf;
}
...
static bool do_statfs(const char *filename, const char *format)
{
	...
	if (statfs(filename, &statfsbuf) != 0) {
		perror_msg("can't read file system information for '%s'", filename);
		return 0;
	}
...
static bool do_stat(const char *filename, const char *format)
{
	...
	if ((option_mask32 & OPT_DEREFERENCE ? stat : lstat) (filename, &statbuf) != 0) {
		perror_msg("can't stat '%s'", filename);
		return 0;
	}
...
extern int stat_main(int argc UNUSED_PARAM, char **argv)
{
