...
//	flags = bb_getopt_ulflags(argc, argv, "ia");	/* 'a' must be 2nd */
	...
	*p = stdout;
	argv += optind - 1;
	filenames = argv - 1;
	*filenames = (char *) msg_standard_input;	/* for later */
	goto GOT_NEW_FILE;

	do {
		if ((*p = wfopen(*argv, mode)) == NULL) {
			retval = EXIT_FAILURE;
			continue;
		}
...
#ifdef CONFIG_FEATURE_TEE_USE_BLOCK_IO
	while ((c = safe_read(_get_osfhandle(STDIN_FILENO), buf, BUFSIZ)) > 0) {
		for (p=files ; *p ; p++) {
			fwrite(buf, 1, c, *p);
		}
	}
