...
static void CDECL set_tty_to_initial_mode(void)
{
	setTermSettings(_fileno(cin), &initial_settings);
}

static void CDECL gotsig(int sig)
{
	putchar('\n');
	exit(EXIT_FAILURE);
}
...
#ifdef BB_FEATURE_USE_TERMIOS
		getTermSettings(_fileno(cin), &initial_settings);
...
#endif
		setTermSettings(_fileno(cin), &new_settings);
...
#if defined BB_FEATURE_AUTOWIDTH && defined BB_FEATURE_USE_TERMIOS
		ioctl(_fileno(stdout), TIOCGWINSZ, &win);
		if (win.ws_row > 4)
