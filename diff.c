...
static __inline dbg_error_msg(char *msg, ...)
{
	error_msg(msg);
}
#endif
...
static int CDECL line_compar(const void *a, const void *b)
{
...
int diff_main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (gotstdin && (S_ISDIR(stb[0].st_mode) || S_ISDIR(stb[1].st_mode)))
		error_msg_and_die("can't compare stdin to a directory");
...
#else
		error_msg_and_die("no support for directory comparison");
#endif
	} else {
