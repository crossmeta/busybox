...
#endif
#if BB_FEATURE_GREP_EGREP_ALIAS
# define OPT_EGREP "E"
#else
# define OPT_EGREP ""
#endif
...
int grep_main(int argc, char **argv)
{
	...
	if (option_mask32 & GREP_OPT_A) {
		lines_after = atoi(slines_after);
	}
	if (option_mask32 & GREP_OPT_B) {
		lines_before = atoi(slines_before);
	}
	/* sanity checks */
	...
	getopt32(argv, GREP_OPTS OPT_EGREP,
		&pattern_head, &fopt);
#endif
	...
	if (BB_FEATURE_GREP_FGREP_ALIAS && applet_name[0] == 'f')
