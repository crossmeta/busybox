		...
		while (*(++(*argv))) {
			...
			case 'a':
				toprint = (PRINT_SYSNAME | PRINT_NODENAME | PRINT_RELEASE |
						   PRINT_PROCESSOR | PRINT_VERSION | PRINT_OS |
						   PRINT_MACHINE);
				break;
			default:
				show_usage();
...
#elif defined(BSD)
{
	...
	mib[0] = CTL_HW;
	mib[1] = HW_MODEL;
	len = sizeof (processor);
	if (sysctl(mib, 2, processor, &len, NULL, 0) == -1) {
			if (errno != EOVERFLOW)
	...
	strcpy(osversion, CONFIG_UNAME_OSNAME);
	...
	print_element(PRINT_OS, osversion);
