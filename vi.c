...
static const char vi_Version[] =
	"$Id: vi.c,v 1.5 2017/03/23 05:33:12 sam Exp $";
...
//WIN32 static fd_set rfds;		// use select() for small sleeps
...
#ifdef BB_FEATURE_VI_USE_SIGNALS
static void CDECL winch_sig(int);	// catch window size changes
static void CDECL suspend_sig(int);	// catch ctrl-Z
static void CDECL alarm_sig(int);	// catch alarm time-outs
static void CDECL catch_sig(int);	// catch ctrl-C
static void CDECL core_sig(int);	// catch a core dump signal
#endif							/* BB_FEATURE_VI_USE_SIGNALS */
#ifdef BB_FEATURE_VI_DOT_CMD
...
#ifdef BB_FEATURE_VI_USE_SIGNALS
static void CDECL winch_sig(int sig)
{
	signal(SIGWINCH, winch_sig);
#ifdef BB_FEATURE_VI_WIN_RESIZE
	window_size_get(0);
...
static void CDECL cont_sig(int sig)
{
...
static void CDECL suspend_sig(int sig)
{
...
static void CDECL catch_sig(int sig)
{
	signal(SIGHUP, catch_sig);
	signal(SIGINT, catch_sig);
	signal(SIGTERM, catch_sig);
...
static void CDECL alarm_sig(int sig)
{
...
static void CDECL core_sig(int sig)
{
	signal(SIGQUIT, core_sig);
	signal(SIGILL, core_sig);
	signal(SIGTRAP, core_sig);
...
static int mysleep(int hund)	// sleep for 'h' 1/100 seconds
{
	...
	int rval;
	fflush(stdout);
	rval = WaitForSingleObject(GetStdHandle(STD_INPUT_HANDLE), hund*10);
...
#else
	while (WaitForSingleObject(GetStdHandle(STD_INPUT_HANDLE), 50) != WAIT_TIMEOUT &&  bufsiz <= (BUFSIZ - 5)) {
#endif
