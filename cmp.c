...
static FILE *cmp_xfopen_input(const char *filename)
{
	...
	if ((fp = wfopen_input(filename)) != NULL) {
		return fp;
	}
...
static const char fmt_differ[] = "%s %s differ: char %I64d, line %I64d\n";
#else
...
int cmp_main(int argc, char **argv)
{
	...
	const char *fmt;
	int c1, c2;
	off64_t char_pos, line_pos;
	int opt_flags;
	int exit_val = 0;

	xfunc_error_retval = 2;	/* 1 is returned if files are different. */

	opt_flags = getopt32(argv, opt_chars);

	if ((opt_flags == 3) || (((unsigned int)(--argc - optind)) > 1)) {
		show_usage();
	}

	fp1 = xfopen_stdin(filename1 = *(argv += optind));
	...
	fflush_stdout_and_exit(exit_val);
}
